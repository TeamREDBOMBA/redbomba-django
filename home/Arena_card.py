...
def cardDetail_sorter(request, league_id):
  ...
  try:
    info = Contents.objects.get(uto=league.id,utotype='l',ctype='inf').con
    info = info.replace("# ","",1)
    info = info.split("\n# ")
    con = {
    "txt":Contents.objects.get(uto=league.id,utotype='l',ctype='txt'),
    "poster":Contents.objects.get(uto=league.id,utotype='l',ctype='img').con,
    "info":info
    }
  except Exception as e:
    con = None
  ...
  try:
    groups = Group.objects.filter(id__in=LeagueTeam.objects.filter(round=LeagueRound.objects.get(league_id=league,round=1)).values_list('group_id', flat=True))
  except Exception as e:
    groups = None
